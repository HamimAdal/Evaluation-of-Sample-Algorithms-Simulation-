...
This case follows a further inspection of how well the space leverages sensors and what effects do the distribution of sensors has on the algorithm’s functionality. To make the problem more challenging, we do not assume the estimated sensor readings at the target location to be perfect. As a matter of fact, the precision of the projected sensor reading at the desired location is entirely dependent on the interpolation method (we used Inverse Distance Weighting Formula) employed to compute the level of illumination. 
